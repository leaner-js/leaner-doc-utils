...
export async function generateFile( file, docsDir, distDir, template, config, files ) {
  let source = await readFile( join( docsDir, file ), 'utf-8' );

  if ( config.tags != null ) {
    for ( const [ tag, value ] of Object.entries( config.tags ) )
      source = source.replaceAll( tag, value );
  }
  ...
  await writeResult( file, distDir, result );

  handleDeadLinks( env.links, files, file );
}

export async function generate404( distDir, template, config ) {
  ...
  await writeResult( '404.md', distDir, result );
}

async function writeResult( file, distDir, result ) {
  const filePath = join( distDir, file.replace( /.md$/, '.html' ) );
